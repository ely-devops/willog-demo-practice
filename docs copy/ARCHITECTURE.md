...
willog-demo는 **Vite + React 19 + TypeScript + TailwindCSS v4** 기반의 물류 추적 및 관리 SaaS 애플리케이션입니다.
...
willog-demo는 **Zustand**를 사용하여 글로벌 상태를 관리합니다. persist 미들웨어를 사용하여 localStorage에 상태를 저장하고, 새로고침 후에도 상태가 복원됩니다.
...
willog-demo는 **React Router v7**을 사용하여 라우팅을 구현합니다. 보호된 라우트 패턴을 적용하여 인증되지 않은 사용자의 접근을 차단합니다.
...
willog-demo는 도메인 개념을 URL 경로에 반영합니다:
...
willog-demo는 다음과 같은 계층화된 컴포넌트 구조를 따릅니다:
...
willog-demo는 **TailwindCSS v4** 기반의 유틸리티 우선 방식을 사용합니다.
